...
Test {
  ...
  return $current->create (
    ...
    return $current->json (['nobj', 'attachform.json'], {
      ...
      byte_length => length ($current->generate_key ('k2' => {})),
    });
  })->then (sub {
    my $result = $_[0];
    $current->set_o (file2 => $result->{json}->{file});
    ...
    return $current->are_errors (
      [['nobj', 'hideunusedattachments.json'], {
        target_nobj_key => $current->o ('c1')->{nobj_key},
        used_url => [
          '',
          'abveeeee',
          $current->o ('file1')->{file_url},
          $current->o ('file2')->{file_url},
        ],
      }],
      [
        ['new_nobj', 'target'],
    ...
    return $current->json (['nobj', 'hideunusedattachments.json'], {
      target_nobj_key => $current->o ('c1')->{nobj_key},
      used_url => [
        '',
        'abveeeee',
    ...
    return $current->client_for ($url)->request (
      ...
      files => {
        file => {body_ref => \($current->o ('k2')), mime_filename => rand},
      },
    ); # upload after hide
  })->then (sub {
    my $res = $_[0];
    die $res unless $res->is_success;
    my $url = Web::URL->parse_string ($current->o ('file2')->{file_url});
    return $current->client_for ($url)->request (url => $url);
  })->then (sub {
    my $res = $_[0];
    test {
    ...
    my $url = Web::URL->parse_string ($current->o ('file2')->{public_file_url});
    return $current->client_for ($url)->request (url => $url);
  })->then (sub {
    my $res = $_[0];
    test {
    ...
    my $url = Web::URL->parse_string ($current->o ('file2')->{signed_url});
    return $current->client_for ($url)->request (
      url => $url,
    );
  })->then (sub {
    ...
    test {
      is $res->status, 200;
      is $res->header ('content-type'), 'application/octet-stream';
      is $res->body_bytes, $current->o ('k2');
    } $current->c, name => 'signed URL';
    return $current->json (['nobj', 'setattachmentopenness.json'], {
      target_nobj_key => $current->o ('c1')->{nobj_key},
      open => 1,
...
Test {
  ...
  return $current->create (
    ...
    test {
      is 0+keys %{$result->{json}->{items}}, 1;
      ok $result->{json}->{items}->{$current->o ('file1')->{file_url}}->{changed};
      ok ! $result->{json}->{items}->{$current->o ('file2')->{file_url}}->{changed};
    } $current->c;
    my $url = Web::URL->parse_string ($current->o ('file1')->{file_url});
    return $current->client_for ($url)->request (url => $url);
  })->then (sub {
